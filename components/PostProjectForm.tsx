  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-2xl">
      ...
      <div>
        ...
        <input className="input" placeholder="Fusion Financials go-live support" value={title} onChange={(e) => setTitle(e.target.value)} required />
      </div>

      <div>
        <label className="text-sm font-semibold text-neutral-800 block mb-1">Description</label>
        <textarea className="input min-h-[140px]" value={description} onChange={(e) => setDescription(e.target.value)} required />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          ...
          <input type="number" min={1} className="input" value={budgetMinGbp} onChange={(e) => setBudgetMinGbp(e.target.value)} />
        </div>
        <div>
          <label className="text-xs text-neutral-600 block mb-1">Budget max (£)</label>
          <input type="number" min={1} className="input" value={budgetMaxGbp} onChange={(e) => setBudgetMaxGbp(e.target.value)} />
        </div>
        <div>
          <label className="text-xs text-neutral-600 block mb-1">Timeline (weeks)</label>
          <input type="number" min={1} className="input" value={timelineWeeks} onChange={(e) => setTimelineWeeks(e.target.value)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input className="input" placeholder="approvals, bpm, notifications" value={tags} onChange={(e) => setTags(e.target.value)} />
        </div>
      </div>
      ...
      {showAdvanced && (
        <div className="card p-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input className="input" placeholder="Business process, e.g. Purchase Orders" value={businessProcess} onChange={(e) => setBusinessProcess(e.target.value)} />
            <input className="input" placeholder="Sub process, e.g. Approval Rules" value={subProcess} onChange={(e) => setSubProcess(e.target.value)} />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <input className="input" placeholder="Oracle version, e.g. 24C" value={oracleVersion} onChange={(e) => setOracleVersion(e.target.value)} />
          </div>

          <p className="text-xs font-semibold text-neutral-700 mt-2">Issue details</p>
          <input className="input" placeholder="Error code (optional)" value={errorCode} onChange={(e) => setErrorCode(e.target.value)} />
          <textarea className="input min-h-[60px]" placeholder="Error message (optional)" value={errorMessage} onChange={(e) => setErrorMessage(e.target.value)} />
          <textarea className="input min-h-[80px]" placeholder="Steps to reproduce (optional)" value={stepsToReproduce} onChange={(e) => setStepsToReproduce(e.target.value)} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <textarea className="input min-h-[60px]" placeholder="Expected behaviour" value={expectedBehaviour} onChange={(e) => setExpectedBehaviour(e.target.value)} />
            <textarea className="input min-h-[60px]" placeholder="Actual behaviour" value={actualBehaviour} onChange={(e) => setActualBehaviour(e.target.value)} />
          </div>

...
export default async function FreelancerOrderHistoryPage({ searchParams }: { searchParams: { page?: string; status?: string } }) {
  ...
  const baseWhere = { gig: { freelancerProfileId: profile!.id } };
  ...
  // genuinely included here regardless of status — cancelled orders were
  // never silently hidden — but there was no way to actually filter down
  // to just one status. A user with many orders had no way to see "just
  // my cancelled ones" or "just what's pending" without scrolling through
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <Link href="/dashboard/freelancer/orders" className={`text-xs px-3 py-1.5 rounded-full border ${!validStatus ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-200 text-neutral-600"}`}>
          All ({allCount})
        </Link>
        {Object.entries(statusLabels).map(([value, label]) => (
          <Link
            key={value}
            href={`/dashboard/freelancer/orders?status=${value}`}
      ...
      {orders.length === 0 ? (
        ...
        <>
          ...
          <Pagination currentPage={page} totalPages={totalPages} basePath="/dashboard/freelancer/orders" searchParams={validStatus ? { status: validStatus } : {}} />
        </>
      )}
    </div>
  );

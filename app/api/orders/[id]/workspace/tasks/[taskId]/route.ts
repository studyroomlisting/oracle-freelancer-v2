...
async function PATCHHandler(req: NextRequest, { params }: { params: { id: string; taskId: string } }) {
  ...
  // RBAC: only the provider (the freelancer delivering the work) can mark a
  // task Done. The client can move a task between To do / In progress
  // (including reopening one the provider marked Done) but can't close it
  // themselves — mirrors a lightweight approval pattern.
  if (parsed.data.status === "DONE") {
    const role = await getOrderWorkspaceRole(session.sub, params.id);
    if (role !== "provider") {
      return NextResponse.json({ error: "Only the freelancer delivering this order can mark a task as Done" }, { status: 403 });
    }
  }

...
// Withdraws a submission before an admin has reviewed it — sends it back
// to DRAFT so the freelancer can keep editing before resubmitting.
async function POSTHandler(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (gig.status !== "PENDING_REVIEW") throw new ApiError("Only a submitted gig awaiting review can be withdrawn", 409);

  const updated = await prisma.gig.update({ where: { id: gig.id }, data: { status: "DRAFT" } });
